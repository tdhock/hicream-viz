...
gg <- ggplot()+
  ...
  theme_bw()+
  geom_tile(aes(
    region2, region1, fill=Value),
    color=NA,
    data=dcast_input[, Value := factor(value)])+
  geom_polygon(aes(
...
png("figure-pixels-chr1-Cluster73-fix.png", width=7, height=3, units="in", res=200)
print(gg)
dev.off()

animint(gg+theme_animint(width=800))
